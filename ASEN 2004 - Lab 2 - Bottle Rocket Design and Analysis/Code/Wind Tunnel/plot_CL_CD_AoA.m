...
%     
%     y_min = min([y_min_CL_clean]) * 1.1;
%     y_max = max([y_max_CL_clean]) * 1.02;
%     
%     
%     sigma_multiplier = 2;
%     
...
%     curr_leg = legend([p1], legend_string, 'location', 'best', ...
%                                      'interpreter', 'latex');
...
%     print(gcf, '-dpdf', saveTitleFull);
    ...
    x_max = 25;
    x_vec = linspace(x_min, x_max, 100);
    speed_vec = [10, 15, 20, 25];
        
    y_min_CD_clean = min(clean.C_D.data);
    
    y_max_CD_clean = max(clean.C_D.data);
    
    y_min = min([y_min_CD_clean]) * 0.8;
    y_max = max([y_max_CD_clean]) * 1.2;
    
    sigma_multiplier = 2;
    
    % find avg C_D
    avg_vec = ones(1, 100) .* mean(clean.C_D.data);
    fprintf('Average C_D: %0.3g\n', mean(clean.C_D.data))
    ...
    %% Plot C_D
    figure_title = sprintf('$C_D$ vs. Airspeed');
    SaveTitleName = 'C_D_vs_Speed';
    saveLocation = '../../Figures/';
    saveTitleFull = cat(2, saveLocation, sprintf('%s.pdf', SaveTitleName));
    xlabel_string = sprintf('Airspeed $[m/s]$');
    ylabel_string = sprintf('$C_D$');
    legend_string = {'Bottle Rocket Data', sprintf('Avg. $C_D$ = %0.3g', mean(clean.C_D.data))};
    ...
    % plot min lines
    p1 = plot(x_vec, avg_vec, '--', 'color', colorVecs(1, :), 'linewidth', LINEWIDTH - 1.5);
       
    
    p2 = errorbar(speed_vec, clean.C_D.data, cleanErrCD, ':^', ...
    ...
    xlabel(xlabel_string)
    ylabel(ylabel_string)
    curr_leg = legend([p2, p1], legend_string, 'location', 'best', ...
                                     'interpreter', 'latex');
    set(curr_leg, 'fontsize', round(FONTSIZE * 0.7));
