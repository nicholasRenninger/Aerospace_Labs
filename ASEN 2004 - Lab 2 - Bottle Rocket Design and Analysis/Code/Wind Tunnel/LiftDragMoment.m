function all_data = LiftDragMoment(all_data, Dist_CG, S, MAC)
    ...
    ten.data = all_data.ten.data;
    ten.error = all_data.ten.error;
    
    fifteen.data = all_data.fifteen.data;
    fifteen.error = all_data.fifteen.error;
    
    twenty.data = all_data.twenty.data;
    twenty.error = all_data.twenty.error;
    ...
    q.data(1) = ten.data(:, data_col);
    q.error(1) = ten.error(:, data_col);
    
    q.data(2) = fifteen.data(:, data_col);
    q.error(2) = fifteen.error(:, data_col);
    
    q.data(3) = twenty.data(:, data_col);
    q.error(3) = twenty.error(:, data_col);
    
    q.data(4) = twentyFive.data(:, data_col);
    q.error(4) = twentyFive.error(:, data_col);
    
    % AoA [deg]
    data_col = 4;
    
    AoA.data(1) = ten.data(:, data_col);
    AoA.error(1) = ten.error(:, data_col);
    
    AoA.data(2) = fifteen.data(:, data_col);
    AoA.error(2) = fifteen.error(:, data_col);
    
    AoA.data(3) = twenty.data(:, data_col);
    AoA.error(3) = twenty.error(:, data_col);
    
    AoA.data(4) = twentyFive.data(:, data_col);
    AoA.error(4) = twentyFive.error(:, data_col);
    ...
    N.data(1) = ten.data(:, data_col) - zero.data(:, data_col);
    N.error(1) = sqrt( ten.error(:, data_col).^2 + ...
                    zero.error(:, data_col).^2 ); % use gen.error formula
    
    N.data(2) = fifteen.data(:, data_col) - zero.data(:, data_col);
    N.error(2) = sqrt( fifteen.error(:, data_col).^2 + ...
                    zero.error(:, data_col).^2 ); % use gen.error formula
    
    N.data(3) = twenty.data(:, data_col) - zero.data(:, data_col);
    N.error(3) = sqrt( twenty.error(:, data_col).^2 + ...
                    twenty.error(:, data_col).^2 ); % use gen.error formula
    
    N.data(4) = twentyFive.data(:, data_col) - zero.data(:, data_col);
    N.error(4) = sqrt( twentyFive.error(:, data_col).^2 + ...
                    zero.error(:, data_col).^2 ); % use gen.error formula
        
    % A [N]
    data_col = 6;
    
    A.data(1) = ten.data(:, data_col) - zero.data(:, data_col);
    A.error(1) = sqrt( ten.error(:, data_col).^2 + ...
                    zero.error(:, data_col).^2 ); % use gen.error formula
    
    A.data(2) = fifteen.data(:, data_col) - zero.data(:, data_col);
    A.error(2) = sqrt( fifteen.error(:, data_col).^2 + ...
                    zero.error(:, data_col).^2 ); % use gen.error formula
    
    A.data(3) = twenty.data(:, data_col) - zero.data(:, data_col);
    A.error(3) = sqrt( twenty.error(:, data_col).^2 + ...
                    twenty.error(:, data_col).^2 ); % use gen.error formula
    
    A.data(4) = twentyFive.data(:, data_col) - zero.data(:, data_col);
    A.error(4) = sqrt( twentyFive.error(:, data_col).^2 + ...
                    zero.error(:, data_col).^2 ); % use gen.error formula
    ...
    M.data(1) = ten.data(:, data_col) - zero.data(:, data_col);
    M.error(1) = sqrt( ten.error(:, data_col).^2 + ...
                    zero.error(:, data_col).^2 ); % use gen.error formula
    
    M.data(2) = fifteen.data(:, data_col) - zero.data(:, data_col);
    M.error(2) = sqrt( fifteen.error(:, data_col).^2 + ...
                    zero.error(:, data_col).^2 ); % use gen.error formula
    
    M.data(3) = twenty.data(:, data_col) - zero.data(:, data_col);
    M.error(3) = sqrt( twenty.error(:, data_col).^2 + ...
                    twenty.error(:, data_col).^2 ); % use gen.error formula
    
    M.data(4) = twentyFive.data(:, data_col) - zero.data(:, data_col);
    M.error(4) = sqrt( twentyFive.error(:, data_col).^2 + ...
                    zero.error(:, data_col).^2 ); % use gen.error formula
