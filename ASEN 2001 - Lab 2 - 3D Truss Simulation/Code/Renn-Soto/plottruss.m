...
for i=1:numelem
    ...
    surf(xc,yc,zc,cc,'EdgeColor','none');
       set(gca,'FontSize', 22)
    if pltflags(2) > 0
        text((xyz(na,1)+xyz(nb,1))/2+1.8*radb,(xyz(na,2)+xyz(nb,2))/2+1.8*radb,(xyz(na,3)+xyz(nb,3))/2+1.8*radb,num2str(i),'FontSize',16);
         set(gca,'FontSize', 22)
    end
    if pltflags(3) > 0
        text((xyz(na,1)+xyz(nb,1))/2+1.8*radb,(xyz(na,2)+xyz(nb,2))/2+1.8*radb,(xyz(na,3)+xyz(nb,3))/2+1.8*radb,sprintf('%.2f',eforce(i)), 'FontSize',16);
        set(gca,'FontSize', 22)
    end
    hold on;
end
...
if pltflags(1) > 0
    for i=1:numnode
            text(xyz(i,1)+1.8*radj,xyz(i,2)+1.5*radj,xyz(i,3)+1.3*radj,num2str(i), 'Color','red', 'FontSize',16);
    end
end
