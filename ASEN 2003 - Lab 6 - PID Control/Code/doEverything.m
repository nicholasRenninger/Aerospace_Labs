...
for j = 1:10
    ...
    if ~isempty(reach5per)
        ...
        saveTitle = ['../Figures/Flexible_TimevAngle' '_Test' num2str(j)];
        ...
        if time(reach5per) > time(timeStart+1000)
            ...
            legend([actual theo td start sec1],{'Actual','Theoretical','Desired Theta','Within 5\%','1 second'},'interpreter', 'latex')
            set(gca, 'defaulttextinterpreter', 'latex')
            set(gca, 'TickLabelInterpreter', 'latex')
            %% label gainz on graph
            annotation('textbox',...
                [0.675 0.46 .18 0.22],...
                'String',{['$K_1$ = ' num2str(K1(1))] ['$K_2$ = ' num2str(K2(1))] ['$K_3$ = ' num2str(K3(1))] ['$K_4$ = ' num2str(K4(1))] ['$\theta _d$ = ' num2str(thetad)]},...
                'FontSize',10,...
                'EdgeColor',[0 0 0],...
                'LineWidth',.01,...
            ...
            legend([actualtip theo2 boundsTip sec12],{'Actual','Theoretical','Maximum Tip Deflection','1 second'},'interpreter', 'latex')
            set(gca, 'defaulttextinterpreter', 'latex')
            set(gca, 'TickLabelInterpreter', 'latex')
            fprintf('Sick Werking Gainz to get Swole for trial %i are K1 = %1.1f K2 = %1.1f K3 = %1.1f K4 = %1.1f\n',j,K1(1),K2(1),K3(1),K4(1));
            saveTitle = ['../Figures/Flexible_TimevTipDeflection' '_Test' num2str(j)];
            saveMeSomeFigs(shouldSaveFigures, saveTitle)
        end
    else
...
for j = 1:10
    ...
    if ~isempty(reach5per)
        ...
        if time(reach5per) > time(timeStart+500)
            ...
            legend([actual theo td start sec5],{'Actual','Theoretical','Desired Theta','Within 5\%','0.5 Seconds'},'interpreter', 'latex')
            set(gca, 'defaulttextinterpreter', 'latex')
            set(gca, 'TickLabelInterpreter', 'latex')
            %% label gainz on graph
            annotation('textbox',...
                [0.675 0.55 .18 0.13],...
                'String',{['$K_p$ = ' num2str(Kp(1))] ['$K_d$ = ' num2str(K3(1))] ['$\theta _d$ = ' num2str(thetad)]},...
                'FontSize',10,...
                'EdgeColor',[0 0 0],...
                'LineWidth',.01,...
                'BackgroundColor',[1 1 1],...
                'Color',[0 0 0],'interpreter', 'latex');
            saveTitle = ['../Figures/Rigid_TimevAngle' '_Test' num2str(j)];
            saveMeSomeFigs(shouldSaveFigures, saveTitle)
        end
